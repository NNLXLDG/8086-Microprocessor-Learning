...
\(\overline{INTA}\) 是CPU发出的低电平有效的中断响应信号 。在中断响应时，CPU通过连续两个中断响应总线周期发出 \(\overline{INTA}\) 信号 。第一个 \(\overline{INTA}\) 信号通知中断源CPU已响应中断，第二个 \(\overline{INTA}\) 信号用于让中断源将中断向量码（中断类型号 ）送到数据总线上供CPU读取 。 
...
#### 6.4.3 中断响应周期
1. **检测中断请求**：每条指令末尾检查INTR/NMI。  
2. **关闭中断**：自动清除IF和TF（防干扰）。  
3. **获取类型号**：通过中断响应总线周期读取类型号。  
4. **跳转至ISR**：根据中断向量表定位入口地址。  
