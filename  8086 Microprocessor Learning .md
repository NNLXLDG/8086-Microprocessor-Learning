...
- [8086 Microprocessor Learning](#8086-microprocessor-learning)
  ...
  - [四 8086 汇编语言程序设计](#四-8086-汇编语言程序设计)
    - [4.1 伪指令](#41-伪指令)
      - [4.1.1 数据定义伪指令](#411-数据定义伪指令)
        - [数据定义伪指令的一般格式](#数据定义伪指令的一般格式)
        - [操作数类型](#操作数类型)
      - [4.1.2 补充内容](#412-补充内容)
        - [类型PTR 地址表达式例如：](#类型ptr-地址表达式例如)
      - [4.1.3 符号定义伪指令](#413-符号定义伪指令)
        - [EQU](#equ)
        - [=(等号)](#等号)
        - [LABEL](#label)
      - [4.1.4 段定义伪指令](#414-段定义伪指令)
        - [段定义伪指令：](#段定义伪指令)
        - [ASSUME](#assume)
        - [ORG](#org)
      - [4.1.5 汇编程序的一般结构](#415-汇编程序的一般结构)
...
2. 汇编以后，每条CPU 指令产生一一对应的目标代码；而伪指令则不产生与之相应的目标代码。
...
##### EQU
格式：名字 EQU 表达式   
EQU伪指令将表达式的值赋予一个名字，以后可以用这个名字代替上述表达式。
``` 
CONSTANT EQU 100
NEW_CONSTANT EQU PORT_VAL+1
```
...
格式：名字 LABEL 类型    
变量的类型可以是BYTE，WORD，DWORD。标号的类型可以是 NEAR 或FAR    
...
附加段及堆栈段，一般是符号、变量定义等伪指令。对于代码段则是指令及伪指令。此外，还必须明确段和段寄存器的关系，这可由ASSUME 语句来实现。   
...
```
ASSUME 伪指令告诉汇编程序，将某一个段寄存器设置为某一个逻辑段址，即明确指出
源程序中逻辑段与物理段之间的关系。



##### ORG
伪指令ORG 规定了段内的起始地址或偏移地址，其格式为：
```
ORG <表达式>
```
