  ...
  - [四 8086 汇编语言程序设计](#四-8086-汇编语言程序设计)
    ...
    - [4.6 DOS及BIOS中断调用](#46-dos及bios中断调用)
      - [4.6.1 中断调用及中断服务子程序返回](#461-中断调用及中断服务子程序返回)
        - [**案例场景**](#案例场景)
    - [**步骤3：中断服务程序执行**](#步骤3中断服务程序执行)
    - [**步骤4：中断返回（执行 `IRET` 恢复现场）**](#步骤4中断返回执行-iret-恢复现场)
  - [三、保护现场示意图](#三保护现场示意图)
    - [**执行 `INT 21H` 时的栈变化**](#执行-int-21h-时的栈变化)
    - [**执行 `IRET` 后的栈恢复**](#执行-iret-后的栈恢复)
  - [四、关键点总结](#四关键点总结)
    - [1. **CPU自动保护的内容**](#1-cpu自动保护的内容)
    - [2. **中断调用与普通子程序调用的区别**](#2-中断调用与普通子程序调用的区别)
    - [3. **现场保护的实际意义**](#3-现场保护的实际意义)
  - [五、结合具体寄存器值的模拟案例](#五结合具体寄存器值的模拟案例)
    - [**假设执行 `INT 21H` 时的初始状态**](#假设执行-int-21h-时的初始状态)
      - [执行 `INT 21H` 后栈内存变化：](#执行-int-21h-后栈内存变化)
      - [恢复时（`IRET`）：](#恢复时iret)
  - [六、延伸思考](#六延伸思考)
...
## 六、延伸思考
- **如何手动保护其他寄存器（如AX、BX）？**  
  若中断服务程序会修改这些寄存器，需要在中断处理程序开头用 `PUSH` 保存，结尾用 `POP` 恢复。
